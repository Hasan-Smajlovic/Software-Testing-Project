...
class ZaraFavouritesPage{
    ...
    async scrollMenuToBottom() {
        const menuSelector = 'a[href="https://www.zara.com/ba/en/woman-l1000.html"]';
        ...
        try {
            ...
            await this.page.evaluate((selector) => {
                ...
                if (menuElement) {
                    menuElement.scrollIntoView({ behavior: 'smooth', block: 'end', inline: 'nearest' });
                }
            }, menuSelector);
    
            console.log('Scrolled to the bottom of the menu.');
        } catch (error) {
            console.error(`Error scrolling to the bottom of the menu: ${error}`);
        }
    }
    async openInfoSpan() {
        const infoSpanSelector = 'span.layout-categories-category__name';
    
        try {
            await this.page.waitForSelector(infoSpanSelector, { timeout: 10000 });
            const infoSpan = await this.page.$(infoSpanSelector);
    
            if (infoSpan) {
                await infoSpan.click();
                await this.page.waitForLoadState('domcontentloaded');
                console.log('Opened the Info span.');
            } else {
                console.error('Info span not found.');
            }
        } catch (error) {
            console.error(`Error opening the Info span: ${error}`);
        }
    }

    async selectAndPressByDataCategoryID(dataCategoryID: string) {
        const elementSelector = `[data-categoryid="${dataCategoryID}"]`;
        ...
        try {
            ...
            if (element) {
                await element.click();  
                await this.page.waitForLoadState('domcontentloaded');
                console.log(`Pressed on element with data-categoryid "${dataCategoryID}".`);
            } else {
                console.error(`Element with data-categoryid "${dataCategoryID}" not found.`);
            }
        } catch (error) {
            console.error(`Error selecting and pressing by data-categoryid "${dataCategoryID}": ${error}`);
        }
    }

    async selectAndClickChatButton() {
        const buttonSelector = 'button.zds-selection-cell__button-wrapper .zds-selection-cell__title';
        ...
        try {
            ...
            console.error(`Error selecting and clicking the Chat button: ${error}`);
        }
    }
